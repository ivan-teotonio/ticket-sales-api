...
export class CustomerModel {
  ...
  static async findByUserId(
    ...
    if (options?.user) {
      query =
        "SELECT c.*, users.id as user_id, users.name as user_name, users.email FROM customers c JOIN users ON c.user_id = users.id WHERE c.user_id = ?";
    }
  ...
  static async findById(
    ...
    if (options?.user) {
      query =
        "SELECT c.*, users.id as user_id, users.name as user_name, users.email FROM customers c JOIN users ON c.user_id = users.id WHERE c.id = ?";
    }
